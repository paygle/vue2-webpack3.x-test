...
:::demo 当`el-form-table`元素中注入`data`对象数组后，在`el-form-table-column`中用`prop`属性来对应对象中的键名即可填入数据，用`label`属性来定义表格的列名。可以使用`width`属性来定义列宽。
```html
  <template>
    <el-form-table
      :rules="rules"
...
| resizable | 对应列是否可以通过拖动改变宽度（需要在 el-form-table 上设置 border 属性为真） | boolean | — | true |
